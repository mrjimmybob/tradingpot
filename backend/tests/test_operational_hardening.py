"""Tests for operational/observability/config fixes: M1, M2, M3, L3, L4.
...
async def _make_db_bot(test_db, strategy="trend_following"):
    bot = Bot(
...
class TestStrategyValidation:
    def test_inverted_dip_recovery_stop_ordering_rejected(self):
        errors = validate_strategy_params(
            "dip_recovery",
            {"trailing_stop_atr_multiplier": 5.0, "emergency_stop_atr_multiplier": 2.0},
        )
        assert any("emergency_stop_atr_multiplier" in e for e in errors)

    def test_valid_dip_recovery_stop_ordering_accepted(self):
        assert validate_strategy_params(
            "dip_recovery",
            {"trailing_stop_atr_multiplier": 1.5, "emergency_stop_atr_multiplier": 5.0},
        ) == []
